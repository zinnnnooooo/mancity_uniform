...
(function () {
  ...
  function productCardHTML(product) {
    ...
    return `
      <a href="product_detail.html?id=${product.id}" class="product-card-link" data-product-id="${product.id}" style="text-decoration: none; color: inherit; display: block;">
  ...
  function initUniformListPage(filter) {
    ...
    if (filter) {
      if (filter.mainTab) currentMainTab = filter.mainTab;
      if (filter.season) currentSeason = filter.season;
      if (filter.type) currentType = filter.type;
      if (filter.collection) currentCollection = filter.collection;
    }
    ...
    const tabs = [
      ...
      { id: "type", el: document.getElementById("tabType") },
      { id: "collection", el: document.getElementById("tabCollection") }
    ];
    tabs.forEach(({ id, el }) => {
      if (!el) return;
      if (id === currentMainTab) {
        el.classList.add("is-active");
        el.setAttribute("aria-selected", "true");
      } else {
  ...
  window.openProductDetail = window.openProductDetail || function (productId) {
  ...
  // Global Event Delegation for Product Card Click
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => {
      if (window.location.pathname.endsWith('uni_list.html')) {
        initUniformListPage(getQueryFilter());
      }
    });
  } else {
    if (window.location.pathname.endsWith('uni_list.html')) {
